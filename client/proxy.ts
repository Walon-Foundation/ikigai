...
export default clerkMiddleware(async (auth, request) => {
  ...
  if (isAdmin) {
    ...
    // root (app/favicon.ico, app/icon.png, app/apple-icon.png, app/manifest.ts,
    ...
    const ADMIN_ASSET_RE =
      /^\/(favicon\.ico|icon(\.png)?|apple-icon(\.png)?|manifest\.webmanifest|sitemap\.xml|robots\.txt|sw\.js|icon-\d+x\d+(-maskable)?\.png)$/;
    if (
